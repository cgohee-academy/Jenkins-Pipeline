...
def after_request(response):
    # Use the matched URL rule for low-cardinality endpoint labeling
    if request.url_rule:
        endpoint = request.url_rule.rule
    else:
        # For 404s and other requests that don't match a rule
        endpoint = 'not_found'
...
def create_todo():
    ...
    try:
        ...
        db.session.rollback()
        structured_logger.log_business_event("todo_creation_failed", {"reason": str(e), "request_data": data}, log_level='ERROR')
...
def update_todo(todo_id):
    ...
    try:
        ...
        db.session.rollback()
        structured_logger.log_business_event("todo_update_failed", {"todo_id": todo_id, "reason": str(e), "request_data": data}, log_level='ERROR')
...
def delete_todo(todo_id):
    ...
    try:
        ...
        db.session.rollback()
        structured_logger.log_business_event("todo_deletion_failed", {"todo_id": todo_id, "reason": str(e)}, log_level='ERROR')
...
def simulate_add_todo_error():
    ...
    structured_logger.log_business_event("todo_creation_failed", {"reason": "simulated_error", "request_data": data}, log_level='ERROR')
...
def simulate_update_todo_error():
    ...
    todo_id = 1  # Use a dummy id for simulation
    data = request.get_json() or {}
    structured_logger.log_business_event("todo_update_failed", {"todo_id": todo_id, "reason": "simulated_error", "request_data": data}, log_level='ERROR')
    structured_logger.log_error("database_error", "Simulated database error on update", context={"operation": "update_todo", "todo_id": todo_id}, log_level='CRITICAL')
    structured_logger.log_database_operation("UPDATE", "todos", False, log_level='CRITICAL')
    return jsonify({'error': f'Failed to update todo {todo_id} (simulated)'}), 500

@app.route('/simulate/delete-todo-error', methods=['DELETE'])
def simulate_delete_todo_error():
    """Simulate a failed todo deletion"""
    todo_id = 1  # Use a dummy id for simulation
    structured_logger.log_business_event("todo_deletion_failed", {"todo_id": todo_id, "reason": "simulated_error"}, log_level='ERROR')
    structured_logger.log_error("database_error", "Simulated database error on delete", context={"operation": "delete_todo", "todo_id": todo_id}, log_level='CRITICAL')
    structured_logger.log_database_operation("DELETE", "todos", False, log_level='CRITICAL')
    return jsonify({'error': f'Failed to delete todo {todo_id} (simulated)'}), 500
